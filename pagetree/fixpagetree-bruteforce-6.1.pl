...
sub traverse {
	...
	while (my ($pageId) = $sth->array) {
		...
		print $pageId."\n";
		$nestPoint++;
		WebGUI::SQL->write("update page set depth=$depth, lft=".quote($nestPoint)." where pageId=".quote($pageId));
		traverse($pageId,$depth);
		$nestPoint++;
		WebGUI::SQL->write("update page set rgt=".quote($nestPoint)." where pageId=".quote($pageId));
	}
	$sth->finish;
}
